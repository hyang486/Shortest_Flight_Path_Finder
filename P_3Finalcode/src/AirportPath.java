...
public class AirportPath implements AirportPathInterface {
    ...
    public AirportPathGraphADTInterface<String, Double> graph;
    ...
    public AirportPath() throws FileNotFoundException {
        // initialize graph with String type node and Double type cost
        graph = new AirportPathGraphADT<String, Double>();
    ...
    public List<String> getShortestPath(String start, String end) {
        ...
        if (graph.containsNode(start) && graph.containsNode(end)) {
            ...
            // if start and end node is same send a error message
            if (start.equals(end)) {
                shortestPath.add("Error! : Start and end airport is same!  " + "Please click reset button");
                return shortestPath;
            } else {// if it is, add the data of the shortest path between start and end
                // airports to List
    ...
    public double getShortestDistance(String start, String end) {
        ...
        if (graph.containsNode(start) && graph.containsNode(end)) {
            ...
            // but if the start and end is same return 0.0 for let user do it again
            if (start.equals(end)) {
                return 0.0;
            } else {
    ...
    public List<String> getShortestTrioPath(String uno, String dos, String tres) {
        ...
        if (graph.containsNode(uno) && graph.containsNode(dos) && graph.containsNode(tres)) {
            if (uno.equals(tres)) {
                shortestTrioPath.add("Error! : Start and end airport is same!  " + "Please click reset button");
                ...
                System.out.println(shortestTrioPath);
                return shortestTrioPath;
            }
        } else {
